...
public class JwtCore {
    ...
    public AuthTokenResponse createAuthToken(String email) {
        log.debug("Creating access+refresh token for email={}", email);
        return new AuthTokenResponse(generateAuthToken(email), generateRefreshToken(email));
    }

    public AuthTokenResponse createAuthToken(String email, String refreshToken) {
        log.debug("Creating access token with existing refresh token for email={}", email);
        return new AuthTokenResponse(generateAuthToken(email), refreshToken);
    }
